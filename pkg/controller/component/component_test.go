...
var _ = Describe("Component", func() {
	...
	Context("has the BuildComponent function", func() {
		...
		PIt("build serviceReference correctly", func() {
			...
			By("call build")
			synthesizeComp, err := BuildSynthesizedComponent(ctx, testCtx.Cli, cluster, compDef, compObj())
			Expect(err).Should(Succeed())
		...
		It("limit the shared memory volume size correctly", func() {
			...
			comp, err := BuildSynthesizedComponent(ctx, testCtx.Cli, cluster, compDef.DeepCopy(), compObj())
			Expect(err).Should(Succeed())
